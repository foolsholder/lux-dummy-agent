...
class Unit:
    ...
    def get_max_battery(self):
        return self.env_cfg.ROBOTS[self.unit_type].BATTERY_CAPACITY

    def check_transfer_location(self, game_state, direction: int) -> bool:
        ...
        if factory_there == -1:
            return False
        if factory_there not in game_state.teams[self.agent_id].factory_strains:
            return False
        return True
